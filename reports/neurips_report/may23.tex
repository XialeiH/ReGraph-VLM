...
We also retrained the no-adjacency gated ROI Transformer directly on the single-reference session-matched pairs and compared it with a retrained ROI-MLP+CLIP baseline (Table~\ref{tab:single_ref_retrained}). This stricter training protocol confirms that the fixed ROI-token transformer advantage is not only an artifact of evaluating checkpoints trained on averaged references.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lccccccc}
    ...
    \midrule
    ROI-MLP+CLIP
    & $0.7759{\scriptstyle\pm0.0460}$
    & $0.7468{\scriptstyle\pm0.0470}$
    & $0.0512{\scriptstyle\pm0.0187}$
    & $0.0447{\scriptstyle\pm0.0136}$
    & $0.0656{\scriptstyle\pm0.0238}$
    & $0.0773{\scriptstyle\pm0.0244}$
    & $0.0621{\scriptstyle\pm0.0163}$ \\
    No-adj gated ROI Transformer+CLIP
    & $\mathbf{0.7873}{\scriptstyle\pm0.0465}$
    & $\mathbf{0.7657}{\scriptstyle\pm0.0480}$
    & $\mathbf{0.0600}{\scriptstyle\pm0.0214}$
    & $\mathbf{0.0504}{\scriptstyle\pm0.0149}$
    & $\mathbf{0.0728}{\scriptstyle\pm0.0265}$
    & $\mathbf{0.0871}{\scriptstyle\pm0.0260}$
    & $\mathbf{0.0688}{\scriptstyle\pm0.0176}$ \\
    \bottomrule
  \end{tabular}
\end{table}

Paired fold$\times$seed tests favor the retrained no-adjacency gated ROI Transformer over retrained ROI-MLP+CLIP for AUROC ($+0.0113$, $p=0.0023$), AUPRC ($+0.0188$, $p=1.72\times10^{-5}$), R@5 ($+0.0088$, $p=3.30\times10^{-5}$), image R@5 ($+0.0072$, $p=0.0106$), and brain R@5 ($+0.0098$, $p=1.66\times10^{-4}$). We interpret this as evidence for the fixed ROI-token transformer and gated readout under stricter session matching, not as evidence that explicit fixed adjacency is necessary.
