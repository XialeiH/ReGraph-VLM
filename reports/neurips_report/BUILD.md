...
The figure asset audit is implemented in `scripts/audit_figure_assets.py`; it
verifies that every manuscript `\IfFileExists` / `\includegraphics` dependency
is present, nonempty, portable, tracked or staged, and bundle-allowlisted, then
writes
`preproc_v0/repetition_familiarity/results/final_tables/figure_asset_audit.csv`.
The dataset accounting audit is implemented in
`scripts/audit_dataset_accounting.py`; it verifies strict T=3 fold accounting,
...
`scripts/audit_external_validation_consistency.py`; it verifies that the dataset
scan, external visual-ROI smoke table, and LAION summary artifacts stay aligned
while preserving the caveat that these are feasibility checks, not full HCP-MMP
