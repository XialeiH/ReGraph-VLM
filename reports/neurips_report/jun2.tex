...
\title{ReGraph-VLM: Fixed-Order ROI-Token Vision Alignment for Cross-Subject Natural-Image fMRI}
...
\begin{equation}
  ...
  + \lambda_{\mathrm{adv}}\mathcal{L}_{\mathrm{subject}}.
\end{equation}
The final main runs use unit weights for BCE and repeat InfoNCE, $\lambda_{\mathrm{clip}}=2.0$, and set the optional cross-subject brain--brain and subject-adversarial terms to zero unless a component baseline explicitly enables them.
...
\begin{equation}
  ...
  \right].
\end{equation}
The identity target in Eq.~(6) treats each positive pair instance as its own contrastive class. If two entries with the same image identity appear in one minibatch, the current implementation does not convert them into a multi-positive contrastive target. We therefore interpret repeat InfoNCE as an auxiliary regularizer and rely on fold-level retrieval metrics, not training loss alone, for the main claims.

The CLIP alignment loss uses the same symmetric contrastive form between brain embeddings and frozen CLIP image embeddings from both sides of each pair. Let $B\in\mathbb{R}^{B\times d}$ be the L2-normalized projected brain embeddings in a minibatch and let $C\in\mathbb{R}^{B\times d}$ be the corresponding L2-normalized projected CLIP image embeddings. In implementation, the two sides of every pair are concatenated, so each brain response is aligned to the CLIP embedding of its own image stimulus. This is true for both positive and negative pair examples: a negative pair contributes two valid brain--image positives for CLIP alignment, while the BCE term marks the brain--brain pair as different-image. The positive index for each brain anchor is its paired image in the same minibatch, and all other images in the minibatch are negatives:
\begin{equation}
...
The default adjacency matrix for adjacency-aware variants is built from training data only for each fold. We stack the training single-trial ROI mean-beta feature into $Z\in\mathbb{R}^{N_{\mathrm{train}}\times180}$, compute the Pearson correlation matrix across ROI columns, zero the diagonal before sparsification, and keep the row-wise top-20 ROI relations ranked by absolute correlation magnitude. The sparse matrix is symmetrized, then model loading converts weights to absolute values, adds diagonal self-connections, and applies symmetric degree normalization $D^{-1/2}AD^{-1/2}$. Validation and test responses are never used to construct adjacency. Adjacency ablations replace this matrix with no-adjacency, identity, shuffled, random, dense-correlation, top-$k$, or edge-dropped variants; these controls are used only to diagnose whether explicit fixed adjacency contributes beyond fixed ROI-token layout.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{lX}
    ...
    Trainable parameters & Final gated BNT/ReGraph and no-adjacency gated ROI Transformer: 3,251,595 parameters each with 512-d CLIP inputs; graph-bias variant: 3,255,883; learned edge-bias variant: 3,283,995; ROI-MLP+CLIP: 369,706 \\
    Loss & BCE + repeat InfoNCE + $\lambda_{\mathrm{clip}}$ CLIP alignment; optional cross-brain InfoNCE and subject-adversarial terms are disabled in final runs \\
    Loss weights & BCE $=1.0$, repeat InfoNCE $=1.0$, $\lambda_{\mathrm{clip}}=2.0$, $\lambda_{\mathrm{cross}}=0$, $\lambda_{\mathrm{adv}}=0$ for final Gated ReGraph-VLM \\
    ...
    Compute transparency & Slurm job scripts and run configs are included in the repository artifacts. GPU model and wall-clock time varied across Shanghai HPC allocations and are not used for model-selection claims. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\end{equation}
Plain R@5/MRR use the model's pair score to rank candidate brain-pair matches. Image R@5 ranks candidate CLIP image embeddings for a query brain embedding. Brain R@5 and brain MRR rank candidate brain embeddings directly in the learned brain-embedding space. Candidate sets are grouped by held-out subject and repeat-pair in the corresponding evaluation split, with one correct same-image target per query; chance R@5 is therefore approximately $5/|\mathcal{C}_q|$ and chance MRR depends on $|\mathcal{C}_q|$. Candidate-pool sizes differ across main, held-out-image, hard-negative, low-shot, and external evaluations, so absolute R@5 values should be compared within a table rather than across protocols. This distinction matters for held-out-image controls: random image anchors can remain competitive for pair discrimination, but should fail on image/brain retrieval if semantic image structure is required.

\paragraph{Statistical reporting convention.}
The primary planned comparison is Gated ReGraph-VLM versus ROI-MLP+CLIP on the main all-fold cross-subject setting, with AUROC and brain R@5 treated as the most important pair-discrimination and retrieval endpoints. We report fold$\times$seed means and standard deviations whenever the underlying repeated evaluations are available. Paired tests use matched fold$\times$seed results, and bootstrap intervals are used for selected retrieval-difference summaries. Because the manuscript contains many secondary diagnostics, ablation tests, and stress tests, their $p$-values are interpreted as exploratory unless explicitly described as primary evidence. We avoid claiming statistical significance for tables that only report means or point summaries.
...
The main averaged-reference benchmark is all-fold cross-subject same-image matching over 8 held-out-subject folds and 3 seeds. Gated ReGraph+CLIP improves over ROI-MLP+CLIP and flat ReGraph+CLIP on all listed metrics (Table~\ref{tab:cross_subject_main}). Paired tests over fold$\times$seed are strong for the comparison against ROI-MLP+CLIP: AUROC difference $+0.0095$ with $p=1.77\times10^{-5}$, AUPRC difference $+0.0168$ with $p=4.66\times10^{-8}$, R@5 difference $+0.0117$ with $p=2.85\times10^{-5}$, and brain R@5 difference $+0.0159$ with $p=2.98\times10^{-7}$. Because this benchmark uses a training-subject averaged reference graph, we treat the single-reference session-matched results in Section~\ref{sec:single_ref_control} as core evidence rather than as an optional diagnostic.
...
For baseline parity, all component baselines use the same serialized train/validation/test folds, the same scalar4 ROI inputs, the same CLIP image embeddings, the same batch size, and the same 8-fold/3-seed evaluation protocol when stable. The comparison is still not a claim against full MindEye2, UMBRAE, or MindLink systems: those methods include reconstruction objectives, voxel-level inputs, additional pretraining, or architecture choices outside this task-matched implementation. We therefore use Table~\ref{tab:sota_baselines} to test whether the core shared-mapper, subject-conditioned, and subject-adversarial ideas solve our specific ROI-token retrieval task under matched data, not to rank full fMRI-to-image pipelines.
...
The main cross-subject tensors compare each held-out-subject anchor response against a training-subject reference graph averaged across available training subjects. This improves reference stability, but it leaves open whether the result depends on averaging away reference-side session structure. To test this, we built a stricter evaluation set in which each positive and matched negative share the same anchor trial, the same reference subject, the same repeat index, and the exact same reference session. We report two versions: an evaluation-only check using the final checkpoints, and a retrained single-reference benchmark using the same stricter pair construction during training. The retrained setting is the more conservative protocol for claims about trial-level cross-subject matching.
...
For the Gated ReGraph/BNT+CLIP checkpoint, moving from the averaged-reference benchmark to the single-reference evaluation-only setting changes AUROC from 0.8259 to 0.7828, AUPRC from 0.8065 to 0.7606, R@5 from 0.0899 to 0.0577, and MRR from 0.0695 to 0.0497. This drop is expected because the reference graph is no longer averaged over training subjects. The important comparison is therefore within each protocol: in both averaged-reference and single-reference settings, the gated ROI-token transformer remains above ROI-MLP+CLIP, while explicit fixed adjacency remains tied with no adjacency.
...
\begin{table}[!htbp]
  \centering
  \caption{LAION-fMRI paired external visual-ROI tests comparing Gated ROI Transformer against ROI-MLP over matched subject-pair$\times$seed runs. Values are paired differences, bootstrap 95\% confidence intervals, and paired-test $p$-values.}
  \label{tab:laion_external_pairwise}
  \scriptsize
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{lrrrr}
    \toprule
    Metric & $n$ & Mean diff. & 95\% CI & Paired $p$ \\
    \midrule
    AUROC & 30 & $-0.0019$ & $[-0.0059,\ 0.0022]$ & $0.3558$ \\
    AUPRC & 30 & $0.0000$ & $[-0.0004,\ 0.0004]$ & $0.8779$ \\
    R@5 & 30 & $0.0093$ & $[-0.0003,\ 0.0189]$ & $0.0575$ \\
    MRR & 30 & $0.0036$ & $[-0.0007,\ 0.0080]$ & $0.0989$ \\
    \bottomrule
  \end{tabular}
\end{table}

All external smoke checks show above-chance cross-subject same-image signal, but they do not replicate the main NSD ordering. In these public compact visual-ROI settings, ROI-MLP is stronger on most aggregate metrics, while the gated ROI Transformer is competitive only on selected retrieval metrics and subject pairs. The LAION-fMRI paired tests make this limitation explicit: there is no significant AUROC or AUPRC difference between the gated ROI Transformer and ROI-MLP, and only a trend toward higher retrieval R@5 for the gated transformer ($+0.0093$, $p=0.0575$; Table~\ref{tab:laion_external_pairwise}). We therefore treat these experiments as external feasibility evidence and as a constraint on the manuscript claim: full external validation should use trial-wise atlas/HCP-MMP beta maps, not only compact visual-localizer or table-format visual ROI summaries. To keep this evidence auditable, the repository includes a separate external-validation consistency check tying Table~\ref{tab:external_visual_roi_smoke}, Table~\ref{tab:laion_external_pairwise}, and the public dataset scan to the same conservative interpretation.

\subsection{Artifact and reproducibility statement}

For anonymous review, the supplementary artifact bundle includes the training code, model definitions, Slurm submission scripts, split-accounting outputs, pair-generation audits, result-summary tables, paired-statistics files, table-value audits, citation/figure audits, external-validation consistency checks, and an anonymous bundle manifest with file-level SHA-256 hashes. The bundle does not include raw NSD beta files or external dataset files because those datasets are governed by their original access terms, but it includes the preprocessing and pair-construction scripts needed to regenerate the serialized inputs after the datasets are obtained. This artifact statement is intended to make the manuscript's claims auditable without exposing author identity.
...
The main result is that Gated ReGraph-VLM improves cross-subject brain graph matching and retrieval over strong baselines in the validated NSD HCP-MMP setting. ROI-MLP+CLIP is a strong non-graph control, flat ReGraph+CLIP is a strong fixed-order transformer baseline, and the task-matched component baselines test recent directions in shared fMRI-to-image mapping, subject-conditioned universal encoding, and subject-adversarial training. These component baselines are useful controls, not full-system comparisons to MindEye2, UMBRAE, or MindLink. The closest component baseline is the MindLink-style subject-adversarial ROI-MLP, but it remains below Gated ReGraph-VLM on AUROC, retrieval R@5/MRR, and brain retrieval.

The stricter single-reference session-matched controls are central to the evidence. The averaged-reference benchmark gives the strongest absolute scores, but it also uses a stabilized reference graph. When we replace this with single-subject reference trials, performance drops as expected, yet the gated ROI-token transformer still improves over ROI-MLP+CLIP. Retraining directly on the single-reference pairs gives the same conclusion. This means the result is not only an artifact of averaged references, but it also shows that the honest final claim is about fixed ROI-token transformer modeling rather than explicit adjacency.

The most important update is that the graph contribution must be framed carefully. The no-adjacency gated ROI Transformer is statistically tied with the final BNT/ReGraph ROI-token variant, while ROI-order shuffling collapses performance and learned gate controls reduce performance. Static adjacency perturbations and learned edge-bias follow-ups do not show a clear advantage over no adjacency. Therefore, the current result should not be presented as evidence in favor of fixed Pearson adjacency. The stronger and safer claim is that fixed anatomical ROI-token layout, learned transformer interactions, gated ROI-preserving readout, and CLIP alignment improve cross-subject retrieval.
...
Overall, the results support a stronger conclusion than simple fMRI graph classification. Gated ReGraph-VLM learns a subject-invariant, image-aligned fixed ROI-token representation that improves cross-subject retrieval in NSD and produces interpretable ROI-level hypotheses. The project therefore contributes both a graph-token VLM method for visual fMRI and a neuroscience framework for studying repeat-stable natural-image representations across subjects.

\section{Conclusion}
\label{sec:conclusion}

We presented \textbf{Gated ReGraph-VLM}, a fixed-order ROI-token brain--vision model for repeated natural-image fMRI. The model encodes HCP-MMP ROI response graphs with a BNT-style ROI transformer, applies a gated anatomy-preserving readout, and aligns brain embeddings with CLIP image embeddings. After initial static image-classification experiments showed that standard graph baselines were insufficient, we reformulated the task as cross-subject same-image brain graph matching and retrieval. In this setting, Gated ReGraph-VLM improves over ROI-MLP+CLIP and flat ReGraph+CLIP in the main all-fold and hard-negative settings, remains favorable under stricter single-reference session-matched controls, and is strongest among real-CLIP models for held-out-image image/brain retrieval. Task-matched decoding component baselines inspired by MindEye2, UMBRAE, and MindLink provide additional context, with the closest competitor being a subject-adversarial ROI-MLP that still underperforms on every key metric.
...
The final contribution is therefore twofold. Methodologically, ReGraph-VLM shows that fixed anatomical ROI-token modeling, gated ROI-preserving readout, and image alignment improve cross-subject brain-image retrieval beyond non-graph, standard graph, and task-matched decoding component baselines in the NSD HCP-MMP setting. Scientifically, it provides a framework for studying how repeated natural images are represented across subjects in visual cortex, and it generates testable predictions about which ROIs should carry cross-subject image-retrieval information in future fMRI experiments.

\section{Limitations}
\label{sec:limitations}

This work has several limitations. We use atlas-level ROI graph features rather than full voxel-level response patterns, which improves interpretability but may discard fine-grained image-specific information. The main dataset contains only eight subjects. We added external smoke checks on public visual-ROI summaries from CNeuroMod-THINGS, BOLD5000, THINGS-fMRI, and LAION-fMRI, but these are not full HCP-MMP external validations and they do not show a consistent gated-transformer advantage over ROI-MLP; broader validation on independent trial-wise atlas-ROI beta maps remains needed. Repetition effects in NSD may reflect adaptation, memory, attention, session structure, or scanner variability, so we avoid interpreting repeat-state decoding as a clean novelty/familiarity measure. The final cross-subject pair tensors are exactly anchor-side session/order matched, and both eval-only and retrained single-reference controls now support the fixed ROI-token conclusion; however, the primary benchmark still uses averaged references and fold\_07 remains an unresolved robustness case. Although held-out-image experiments support the value of real CLIP embeddings for image/brain retrieval, shuffled and random embedding controls suggest that part of the auxiliary image-alignment objective may also act as regularization. The task-matched component baselines are not full-system reproductions of recent fMRI-to-image decoders, and runtime/GPU metadata were not standardized across all exploratory HPC jobs. Finally, the learned gates provide model-level explanations rather than causal neuroscience evidence; future work should test voxel-level features, stronger session controls, edge-level mechanisms, and external dataset generalization.
