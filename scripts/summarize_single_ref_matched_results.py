...
def mean_std_values(values: list[float]) -> tuple[float, float]:
    arr = np.asarray(values, dtype=float)
    arr = arr[np.isfinite(arr)]
    if arr.size == 0:
        return math.nan, math.nan
    if arr.size == 1:
        return float(arr.mean()), math.nan
...
def latex_metric(mean: float, std: float) -> str:
...
def write_latex(path: Path, rows: list[dict[str, Any]]) -> None:
    lines = [
        ...
        "% Values are mean \\pm std over fold x seed runs.",
    ]
    for model in sorted({row["model"] for row in rows}):
        subset = [row for row in rows if row["model"] == model]
        values = []
        for metric in METRICS:
            mean, std = mean_std_values([float(row[metric]) for row in subset])
            values.append(latex_metric(mean, std))
...
def main() -> None:
    ...
    write_latex(args.output_dir / f"{prefix}_latex.txt", rows)
