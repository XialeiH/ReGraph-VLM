...
train_job=$(sbatch --parsable --dependency=afterok:"$export_job" scripts/shanghai_laion_fmri_matched_public_roi_array.sbatch)
echo "Submitted LAION matched-public-ROI training array: $train_job"

summary_job=$(sbatch --parsable --dependency=afterok:"$train_job" scripts/shanghai_laion_fmri_matched_public_roi_summary.sbatch)
echo "Submitted LAION matched-public-ROI summary job: $summary_job"
