...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate train-only normalization and shared PCA artifacts for one all8_ge2_766 fold.")
...
def fit_pca(train_matrix: np.ndarray, pca_dim: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    np.savez(
        ...
        dataset_view=np.array("all8_ge2_766"),
    ...
    train_index_path = fold_prefix.with_name(f"{args.fold_name}_train_features_index.csv")
    test_index_path = fold_prefix.with_name(f"{args.fold_name}_test_features_index.csv")
    for path, rows in [(train_index_path, train_index_rows), (test_index_path, test_index_rows)]:
    ...
    (fold_prefix.with_name(f"{args.fold_name}_pca_diagnostics.json")).write_text(
    ...
    note_lines = [
        ...
        "",
        "- Primary dataset view: `all8_ge2_766`.",
        "- Version 0 main view uses all 8 subjects and requires at least 2 repetitions per subject-image sample.",
        "- Normalization is training-only feature-wise z-score on a canonical ROI-block-padded concatenated feature space.",
        "- Canonical ROI blocks are formed by padding each ROI to the maximum ROI dimension observed across subjects in `all8_ge2_766`.",
        ...
        "- These artifacts are intended to unblock B1 and B4 baselines on a clean LOSO setup.",
        "",
    ]
