...
This regenerates lightweight result artifacts, runs the AAAI artifact audit, runs the full manuscript/result audit, checks README/BUILD consistency, verifies key manuscript table values and statistical claims against committed CSV artifacts, runs the manuscript-only audit, and reports whether a local TeX compiler is available.
